...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <h2 class="text-2xl font-bold text-slate-100">Tulis Artikel Baru</h2>
    </div>

    <form action="{{ route('admin.articles.store') }}" method="POST" enctype="multipart/form-data" class="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-sm space-y-6">
        @csrf
        
        <div>
            <label class="block text-sm font-medium text-slate-300 mb-2">Judul Artikel</label>
            <input type="text" name="title" value="{{ old('title') }}" required class="w-full bg-slate-950 border border-slate-800 rounded-lg px-4 py-2.5 text-slate-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
