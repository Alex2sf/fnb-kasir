    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="flex-shrink-0 w-64 bg-slate-900 border-r border-slate-800 flex flex-col">
            ...
            <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
                ...
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.dashboard') ? 'bg-indigo-500/10 text-indigo-400 font-semibold' : 'text-slate-400 hover:bg-slate-800 transition-colors' }}">
                    <i data-lucide="layout-dashboard" class="w-5 h-5"></i>
                    Dashboard
                </a>

                <div class="pt-5 pb-2 px-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Manajemen</div>
                
                <a href="{{ route('admin.articles.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg {{ request()->routeIs('admin.articles.*') ? 'bg-indigo-500/10 text-indigo-400 font-semibold' : 'text-slate-400 hover:bg-slate-800 transition-colors' }}">
