        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div x-show="showModal" x-transition class="relative inline-block w-full max-w-md p-6 overflow-hidden text-left align-middle transition-all transform bg-white shadow-2xl rounded-md border border-slate-200">
                ...
                <form :action="editMode ? '{{ url('owner/discounts') }}/' + discountId : '{{ route('owner.discounts.store') }}'" method="POST">
                    ...
                    <div class="space-y-5">
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Nama Promo</label>
                            <input type="text" name="name" x-model="name" required class="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all" placeholder="Contoh: Promo Ramadhan">
                        </div>
                        
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Tipe Potongan</label>
                            <select name="type" x-model="type" class="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Besaran Potongan</label>
                            <div class="relative">
                                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 font-medium" x-text="type === 'fixed' ? 'Rp' : '%'"></span>
                                <input type="number" name="value" x-model="value" required class="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all">
                            </div>
                            <p class="text-xs text-slate-500 mt-2" x-text="type === 'fixed' ? 'Isi tanpa titik, misal 15000 untuk Rp 15.000' : 'Isi angka persen, misal 10 untuk Diskon 10%'"></p>
                        </div>

                        <div class="flex items-center gap-3 pt-2">
                            <input type="checkbox" name="is_active" id="is_active" x-model="is_active" class="w-5 h-5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                            <label for="is_active" class="text-sm font-semibold text-slate-700">Aktifkan promo ini sekarang</label>
                        </div>
                    </div>
                    ...
                    <div class="mt-8 flex justify-end gap-3 pt-4 border-t border-slate-100">
                        ...
                        <button type="submit" class="px-5 py-2.5 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-md transition-colors shadow-sm" x-text="editMode ? 'Simpan Perubahan' : 'Tambah Promo'"></button>
                    </div>
                </form>
            </div>
        </div>
